(function() {
  ...
  const EXCHANGE_API = 'https://v6.exchangerate-api.com/v6/45602791eb9f29e022a7ce3f/latest/USD';
  const FINNHUB_API = 'https://finnhub.io/api/v1/news?category=general&token=d7cq1t9r01qv03eta4rgd7cq1t9r01qv03eta4s0';
  const ALPHAVANTAGE_API = 'https://www.alphavantage.co/query';
  const ALPHAVANTAGE_KEY = '3A3XN5PXKW8DG38L';
  
  // EmailJS Configuration
  const EMAILJS_SERVICE_ID = 'service_8q0qj4p';
  const EMAILJS_TEMPLATE_ID = 'template_0bqt6ca';
  const EMAILJS_AUTOREPLY_TEMPLATE_ID = 'template_f173we8';
  const EMAILJS_USER_ID = 'nI5fzMuQFZPx7aq9T';
  const ADMIN_EMAIL = 'online-base@hotmail.com';
  ...
  const DB = {
    ...
    // Global storage (shared across accounts) for user registration data
    getGlobal(key, def) {
      try {
  ...
  const Auth = {
    ...
    setSession(token, user) {
      ...
      const allUsers = DB.getGlobal('users', []);
      const idx = allUsers.findIndex(u => u.email === user.email);
      if (idx !== -1) {
        allUsers[idx].lastLogin = new Date().toISOString();
        allUsers[idx].loginLocation = user.loginLocation || allUsers[idx].loginLocation;
        allUsers[idx].loginLanguage = user.loginLanguage || allUsers[idx].loginLanguage;
        DB.setGlobal('users', allUsers);
      }
    },
    logout() {
      DB.remove('token');
  ...
  window.TS_LocationDetector = LocationDetector;
  ...
  function injectGlobalUI() {
    ...
    if (!document.getElementById('ts-mt5-styles')) {
      ...
      style.textContent = `
        ...
        body { background: var(--bg-primary); margin: 0; font-family: system-ui, -apple-system, sans-serif; }
        ...
        .ts-more-item{display:flex;align-items:center;gap:10px;padding:10px 14px;color:#c1c1c1;font-size:13px;text-decoration:none;transition:background .12s;border-bottom:1px solid #1a1a1a;background:transparent;border:none;cursor:pointer;width:100%;text-align:left;}
        ...
        .ts-notification-banner{position:sticky;top:48px;z-index:55;background:linear-gradient(135deg,#1a1a2e,#16213e);border:1px solid #30363d;border-radius:0 0 12px 12px;padding:12px 16px;margin:0 8px;display:flex;align-items:center;gap:12px;box-shadow:0 8px 24px rgba(0,0,0,.4);}
        .ts-notification-banner-content{flex:1;min-width:0;}
        .ts-notification-banner-title{font-size:12px;font-weight:700;color:#e6edf3;margin-bottom:2px;}
        .ts-notification-banner-text{font-size:11px;color:#8b949e;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
        .ts-notification-banner-close{background:transparent;border:none;color:#8b949e;cursor:pointer;padding:4px;flex-shrink:0;}
        .ts-notification-banner-close:hover{color:#f85149;}
      `;
    ...
    if (header && isLogged) {
      ...
      // Notification Banner
      const notifBanner = document.getElementById('ts-notification-banner');
      if (notifBanner && !DB.get('notif_banner_dismissed', false)) {
        // Will be populated by notification system
      }
    }
  ...
  window.TS_Market = {
    async getCoins() {
      ...
      try {
        const res = await fetch(`${COINGECKO_API}/coins/markets?vs_currency=usd&per_page=100&page=1&sparkline=false`);
        const data = await res.json();
    ...
    async getNews() {
      ...
      try {
        const res = await fetch(FINNHUB_API);
        const data = await res.json();
    ...
    async getAlphaVantage(symbol) {
      try {
        const res = await fetch(`${ALPHAVANTAGE_API}?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=${ALPHAVANTAGE_KEY}`);
        return await res.json();
      } catch {
        return null;
      }
  ...
  window.TS_Invest = {
    ...
    addTrade(trade) {
      ...
      TS_Activity.add('trade', `New trade opened: ${trade.symbol}`, trade.amount);
      return trade;
    },
    ...
    addHolding(holding) {
      ...
      TS_Activity.add('holding', `New holding added: ${holding.planName}`, holding.amount);
      return holding;
    },
      ...
      const trades = this.getTrades().map(t => {
        if (t.status === 'active') {
          ...
          if (Date.now() >= end) {
            ...
            TS_Activity.add('trade_complete', `Trade completed with profit: $${t.profit.toFixed(2)}`, t.profit);
          }
        }
        return t;
      });
      ...
      const holdings = this.getHoldings().map(h => {
        if (h.status === 'active') {
          ...
          if (Date.now() >= end) {
            ...
            TS_Activity.add('holding_complete', `Holding completed with profit: $${h.profit.toFixed(2)}`, h.profit);
          }
        }
        return h;
      });
  ...
  // ======================= ACTIVITY LOG =======================
  window.TS_Activity = {
    getAll() { return DB.get('activities', []); },
    add(type, description, amount) {
      ...
      DB.set('activities', activities.slice(0, 100));
    }
  };
  ...
  // ======================= MAILBOX =======================
  window.TS_Mailbox = {
    getAll() { return DB.get('mailbox_messages', []); },
    add(msg) {
      ...
      DB.set('mailbox_messages', messages.slice(0, 200));
      return msg;
    },
    markRead(id) {
      const messages = this.getAll().map(m => m.id === id ? { ...m, read: true } : m);
      DB.set('mailbox_messages', messages);
    },
    delete(id) {
    ...
    deleteMultiple(ids) {
      const messages = this.getAll().filter(m => !ids.includes(m.id));
      DB.set('mailbox_messages', messages);
    },
    markMultipleRead(ids) {
      const messages = this.getAll().map(m => ids.includes(m.id) ? { ...m, read: true } : m);
      DB.set('mailbox_messages', messages);
    }
  };
  ...
  window.TS_Balance = {
    ...
    subtract(val) { this.set(Math.max(0, this.get() - val)); },
    getTotalDeposits() { return DB.get('total_deposits', 0); },
    getTotalWithdrawals() { return DB.get('total_withdrawals', 0); },
    addDeposit(val) {
      this.add(val);
      DB.set('total_deposits', this.getTotalDeposits() + val);
      TS_Activity.add('deposit', `Deposit received: $${val.toFixed(2)}`, val);
    },
    addWithdrawal(val) {
      this.subtract(val);
      DB.set('total_withdrawals', this.getTotalWithdrawals() + val);
      TS_Activity.add('withdrawal', `Withdrawal processed: $${val.toFixed(2)}`, val);
    },
    getNetProfit() {
  ...
  // ======================= SETTINGS =======================
  window.TS_Settings = {
    get(key, def) { return DB.get('setting_' + key, def); },
    set(key, val) { DB.set('setting_' + key, val); },
    toggle(key) {
  ...
  window.selectLeverage = selectLeverage;
  ...
  window.goStep = function(step) {
    ...
    if (step > currentStep) {
      if (currentStep === 1) {
        ...
        if (!firstName || !lastName || !email || !phone) {
          TS_UI.showToast('Please fill in all personal details', 'warning');
          return;
        }
        if (!email.includes('@') || !email.includes('.')) {
          TS_UI.showToast('Please enter a valid email address', 'warning');
          return;
        }
      }
      if (currentStep === 2) {
        const selectedAcct = document.querySelector('.ts-acct-card.selected');
        if (!selectedAcct) {
          TS_UI.showToast('Please select an account type', 'warning');
          return;
        }
      }
    }
    ...
    // Update wizard lines
    document.querySelectorAll('.ts-step-line').forEach(el => {
      const lineNum = parseInt(el.id.replace('line-', ''));
      if (lineNum < step) el.classList.add('active');
  ...
  window.submitRegistration = async function() {
    ...
    if (!firstName || !lastName || !email || !phone || !password || !confirmPassword) {
      TS_UI.showToast('Please fill in all required fields', 'warning');
      return;
    }
    ...
    if (password.length < 6) {
      TS_UI.showToast('Password must be at least 6 characters', 'warning');
      return;
    }
    
    if (!terms) {
      TS_UI.showToast('Please agree to the terms and conditions', 'warning');
      return;
    }
    
    const selectedAcct = document.querySelector('.ts-acct-card.selected');
    const selectedCurrency = document.querySelector('#currency-badges .ts-badge.active');
    const selectedLeverage = document.querySelector('#leverage-badges .ts-badge.active');
    
    const accountType = selectedAcct ? selectedAcct.dataset.type : 'Standard';
    const currency = selectedCurrency ? selectedCurrency.dataset.currency : 'USD';
    const leverage = selectedLeverage ? selectedLeverage.dataset.leverage : '1:100';
    
    // Detect location
    const locationData = await LocationDetector.detect();
    
    // Generate 10-digit user account ID
    const userId = String(Math.floor(1000000000 + Math.random() * 9000000000));
    ...
    const userData = {
      ...
      loginLanguage: locationData.language,
      avatar: null
    };
    ...
    const progressInterval = setInterval(() => {
      ...
      if (loaderDetail) {
        if (progress < 30) loaderDetail.textContent = 'Validating information...';
        else if (progress < 60) loaderDetail.textContent = 'Creating account...';
        else if (progress < 80) loaderDetail.textContent = 'Connecting to trade server...';
        else loaderDetail.textContent = 'Finalizing setup...';
      }
    }, 300);
    ...
    setTimeout(() => {
      ...
      if (loaderDetail) loaderDetail.textContent = 'Account created successfully!';
      
      setTimeout(() => {
        // Set session
        Auth.setSession('local-token-' + userId, userData);
        
        // Initialize account data
        DB.set('balance', 0);
        DB.set('total_deposits', 0);
        DB.set('total_withdrawals', 0);
        DB.set('trades', []);
        DB.set('holdings', []);
        DB.set('activities', []);
        ...
        TS_Notifications.add({
          ...
          message: `Your account has been created successfully. Account ID: ${userId}`,
          type: 'success'
        });
        ...
        TS_Mailbox.add({
          from: 'Trade Station',
          subject: 'Welcome to Trade Station',
          body: `Dear ${firstName},\n\nYour trading account has been successfully created.\n\nAccount ID: ${userId}\nAccount Type: ${accountType}\nCurrency: ${currency}\nLeverage: ${leverage}\n\nStart trading today and grow your portfolio!\n\nBest regards,\nTrade Station Team`,
          type: 'system'
        });
  ...
  window.saveAccountData = function() {
    const user = Auth.getUser();
    if (!user) {
      TS_UI.showToast('No user session found', 'error');
      return;
    }
    ...
    if (idx !== -1) {
      ...
      TS_UI.showToast('Account data saved', 'success');
    }
  };
  ...
  window.handleLogin = async function(email, password) {
    ...
    try {
      ...
      if (data.token) {
        ...
        setTimeout(() => { window.location.href = 'dashboard.html'; }, 15000);
        return;
      }
    } catch {}
    ...
    if (user) {
      ...
      user.loginLocation = locationData;
      user.loginLanguage = locationData.language;
      ...
      if (loadingOverlay) loadingOverlay.classList.add('hidden');
      TS_UI.showToast('Invalid email or password. Please check your credentials.', 'error');
    }
  };
  ...
  async function initNotificationBanner() {
    ...
    if (DB.get('notif_banner_dismissed', false)) return;
    
    try {
      const news = await TS_Market.getNews();
      const banner = document.getElementById('ts-notification-banner');
      if (banner && news && news.length > 0) {
        const latestNews = news[0];
        banner.innerHTML = `
          <div class="ts-notification-banner">
            <div class="ts-notification-banner-content">
              <div class="ts-notification-banner-title">📰 Market Update</div>
              <div class="ts-notification-banner-text">${latestNews.headline || latestNews.title || 'Live market news update available'}</div>
            </div>
            <button class="ts-notification-banner-close" onclick="dismissNotificationBanner()">
              ${ICONS.close}
            </button>
          </div>
        `;
  ...
  window.dismissNotificationBanner = function() {
    DB.set('notif_banner_dismissed', true);
    const banner = document.getElementById('ts-notification-banner');
  ...
  window.TS_Auth = Auth;
  window.TS_API = api;
  window.TS_UI = window.TS_UI;
  window.TS_Market = window.TS_Market;
  window.TS_Invest = window.TS_Invest;
  window.TS_Balance = window.TS_Balance;
  window.TS_Withdraw = window.TS_Withdraw;
  window.TS_Deposit = window.TS_Deposit;
  window.TS_Notifications = window.TS_Notifications;
  window.TS_Mailbox = window.TS_Mailbox;
  window.TS_Activity = window.TS_Activity;
  window.TS_Settings = window.TS_Settings;
  window.TS_LocationDetector = LocationDetector;
